...
namespace omm
{
	...
	public:
		...
		explicit constexpr Vec2(const SkPoint& p) : x(p.x()), y(p.y())
		{
		}
		...
		[[nodiscard]] SkPoint to_geom_point() const
		{
	...
	template<typename ValueT>
	Vec2<ValueT> operator+(const Vec2<ValueT>& d1, const Vec2<ValueT>& d2)
	{
	...
	template<typename ValueT>
	Vec2<ValueT> operator-(const Vec2<ValueT>& d, const ValueT& s)
	{
	...
	template<typename ValueT>
	Vec2<ValueT> operator*(const ValueT s, const Vec2<ValueT>& d)
	{
		return d * s;
	}
	...
	template<typename ValueT>
	Vec2<ValueT> operator*(const Vec2<ValueT>& s, const Vec2<ValueT>& d)
	{
		return Vec2<ValueT>(s.x * d.x, s.y * d.y);
	}

	template<typename ValueT>
	Vec2<ValueT> operator/(const Vec2<ValueT>& d, const Vec2<ValueT>& s)
	{
		return Vec2<ValueT>(d.x / s.x, d.y / s.y);
	}

	template<typename ValueT>
	bool operator==(const Vec2<ValueT>& d1, const Vec2<ValueT>& d2)
	{
		return d1.x == d2.x && d1.y == d2.y;
	}
 ...
 */
	...
	template<typename T>
	bool fuzzy_eq(const Vec2<T>& a, const Vec2<T>& b)
	{
