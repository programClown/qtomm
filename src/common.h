...
bool any_of(const Ts& ts, const F& f)
{
...
template<typename SetA, typename SetB = SetA>
decltype(auto) merge(SetA&& a, SetB&& b)
{
...
template<typename SetA, typename SetB = SetA>
SetA merge(const SetA& a, SetB&& b)
{
	auto copy = a;
...
bool contains(const std::map<Ts...>& map, S&& key)
{
	if constexpr (std::is_pointer_v<S> || std::is_reference_v<S>) {
		// NOLINTNEXTLINE(cppcoreguidelines-pro-type-const-cast)
...
private:
	...
	template<typename T>
	static constexpr auto check(T*) -> U<T>;
	template<typename>
...
constexpr bool typelist_contains() noexcept
{
...
std::unique_ptr<T> copy_unique_ptr(const std::unique_ptr<T>& up, Args&& ... args)
{
	if (up) {
...
namespace omm
{
	...
	requires EnableBitMaskOperators<EnumT>::value EnumT operator|(EnumT a, EnumT b)
	{
	...
	template<typename EnumT>
	requires EnableBitMaskOperators<EnumT>::value EnumT operator~(EnumT a)
	{
		return static_cast<EnumT>(~static_cast<std::underlying_type_t<EnumT>>(a));
	}

	template<typename EnumT>
	requires EnableBitMaskOperators<EnumT>::value bool operator!(EnumT a)
	{
 ...
 */
	...
	auto find_coherent_ranges(const Vs& vs, F&& f)
	{
		struct Range
		{
			std::size_t start;
	...
	template<typename T>
	auto python_like_mod(const T& dividend, const T& divisor)
	{
...
template<>
struct omm::EnableBitMaskOperators<omm::Kind> : std::true_type
{
};
template<>
struct omm::EnableBitMaskOperators<omm::Flag> : std::true_type
{
};
