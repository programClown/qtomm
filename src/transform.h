...
namespace util
{
	...
	namespace transform_detail
	{
		...
		void push_back(C& cs, T&& val)
		{
			cs.push_back(std::forward<T>(val));
		}

		template<has_insert C, typename T>
		void push_back(C& cs, T&& val)
		{
...
// Base implementation for associative containers that need to be moved
		...
		{
			return transform_helper<ValueType, ReturnContainer, ConcreteContainer, F>(std::forward<decltype(cs)>(cs),
				f);
		}
